...
void adc_measure(){
   ...
   *AT91C_ADCC_IER = (1<<24);                       //Enable interrupt (DRDY: Data Ready Interrupt Enable) on the ADC
   ...
   while(1)
   {
       if(light_sens_ch1_ready ==1 && ~(adc_read_status&0x1))
       {
           adc_CDR1_value = (0xFFF&(*AT91C_ADCC_LCDR));             //Data is contained from bit 0 to 11, 0xFFF & LCDR extracts all relevant data
           adc_read_status|=0x1;
       }
       if(light_sens_ch2_ready ==1 && ~(adc_read_status&0x2))
       {
           adc_CDR2_value = (0xFFF&(*AT91C_ADCC_LCDR));             //Data is contained from bit 0 to 11, 0xFFF & LCDR extracts all relevant data
           adc_read_status|=0x2;
       }
